...
cc.Class({
    ...
    start () {
        ...
        this.max = Global.Model.MEnergy.getAttr().maxNum;
        this.interval = Global.Model.MEnergy.getAttr().interval;
        this.generateRate = Global.Model.MEnergy.getAttr().rate;
    },
    ...
    generateMonster:function (parent , maxNum , origin , range) {
        ...
        var basePosx = origin + Global.Common.Utils.random(this.startGenerate , this.startGenerate + range);
        for (let index = 0; index < len; index++) {
            var posx = basePosx + 200 * index;
            // 判断怪物是否生成在墙内
