 ...
 * 普通怪物数据初始化
 */
MFlyCoins.init = function () {
    // 类型
    this.type = Enum.TYPE.COINS_FLY;
    // 怪物属性
    this.attr = {};
    // 弹性
    this.attr.elastic = 0;
...
MFlyCoins.setConfig = function (config) {
    this.config = config;
};
 ...
 */
MFlyCoins.updateByPass = function (passID) {
    var data = null;
    ...
    var cfg = Global.Model.Game.getConfigByLevel(Global.Model.Game.levelsItemConfig.flycoins , level);
    ...
    console.log("UpdateByPass MFlyConis" , passID);
};
 ...
 */
MFlyCoins.handleCollision = function (contact , selfCollider , otherCollider) {
 ...
 */
MFlyCoins.collisionFloor = function (contact , monsterCollider , floorCollider) {
 ...
 */
MFlyCoins.collisionWall = function (contact , monsterCollider , wallCollider) {
    var monster = monsterCollider.node.getComponent(Global.GameObj.GBase);
 ...
 * @param {any} x 速度值
 */
MFlyCoins.limitVelocityX = function (x) {
