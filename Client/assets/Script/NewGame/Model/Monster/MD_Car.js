...
MCar.setConfig = function (config) {
    this.config = config;
};
...
MCar.updateByPass = function (passID) {
    ...
    var level = Global.Model.Game.getLevelByItemID(11);
    var cfg = Global.Model.Game.getConfigByLevel(Global.Model.Game.levelsItemConfig.car , level);
    ...
    console.log("UpdateByPass MCar" , passID);
};
 ...
 */
MCar.handleCollision = function (contact , selfCollider , otherCollider) {
 ...
 */
MCar.collisionFloor = function (contact , monsterCollider , floorCollider) {
 ...
 */
MCar.collisionWall = function (contact , monsterCollider , wallCollider) {
    var monster = monsterCollider.node.getComponent(Global.GameObj.GBase);
 ...
 * @param {any} x 速度值
 */
MCar.limitVelocityX = function (x) {
 ...
 * @param {any} y 速度值
 */
MCar.limitVelocityY = function (y) {
