...
MFly.init = function () {
    // 类型
    this.type = Enum.TYPE.NORMAL_FLY;
    // 怪物属性
    this.attr = {};
    // 弹性
    this.attr.elastic = 0;
...
MFly.setConfig = function (config) {
    this.config = config;
};
...
MFly.updateByPass = function (passID) {
    ...
    console.log("UpdateByPass MFly" , passID);
};
 ...
 */
MFly.handleCollision = function (contact , selfCollider , otherCollider) {
 ...
 */
MFly.collisionFloor = function (contact , monsterCollider , floorCollider) {
 ...
 */
MFly.collisionWall = function (contact , monsterCollider , wallCollider) {
    var monster = monsterCollider.node.getComponent(Global.GameObj.GBase);
 ...
 * @param {any} x 速度值
 */
MFly.limitVelocityX = function (x) {
 ...
 * @param {any} y 速度值
 */
MFly.limitVelocityY = function (y) {
